        ...
        >
          <div>
            <span style={{ fontSize: "16px", fontWeight: 700, color: "white" }}>
              Upload Qualys Report
            </span>
            <div
              style={{ fontSize: "11px", color: "#52525b", marginTop: "3px" }}
            >
              Step {step} of 2
            </div>
          </div>
          ...
          {[1, 2].map((n) => (
            <div
              ...
              style={{
                ...
                height: "3px",
                borderRadius: "2px",
                background: step >= n ? "#818cf8" : "#1f1f2e",
        ...
        {step === 1 ? (
          <>
            ...
            >
              ...
              this Qualys report belongs to. This links vulnerabilities to your
              scanned assets.
            </p>
            <label
              style={{
            ...
            {assetScans.length === 0 ? (
              ...
              >
                No asset scans found. Create one in{" "}
                <strong>Asset Scanning</strong> first.
              </div>
            ) : (
              <select
              ...
              >
                ...
                {assetScans.map((s) => (
                  <option key={s.id} value={s.id}>
                    {s.scan_name || s.filename} ({s.total_assets} assets)
                  </option>
                ))}
              </select>
            )}
            <button
              onClick={() => {
                if (!selectedScanId) {
                  setError("Please select an asset scan.");
                  return;
                }
                setError(null);
                setStep(2);
              ...
              style={{
                ...
                width: "100%",
                padding: "10px",
                background: assetScans.length === 0 ? "#1f1f2e" : "#818cf8",
                color: assetScans.length === 0 ? "#52525b" : "white",
                fontWeight: 700,
                fontSize: "13px",
                border: "none",
                borderRadius: "8px",
                cursor: assetScans.length === 0 ? "default" : "pointer",
              }}
            >
              Continue →
            </button>
          ...
          <>
            ...
            <div
              ...
              style={{
                border: `2px dashed ${dragging ? "#818cf8" : "#2a2a3a"}`,
                borderRadius: "12px",
                padding: "32px 24px",
                display: "flex",
            ...
            >
              ...
              >
                {uploading
                  ...
                  : dragging
                    ? "Drop it!"
                    : "Drop Qualys report or click to browse"}
              </p>
              <p
                style={{
                  fontSize: "11px",
...
export function AssetScanBadge({ ip }: { ip: string }) {
  ...
  if (rows === null)
    return <span style={{ color: "#52525b", fontSize: "11px" }}>…</span>;
  if (rows.length === 0)
    return <span style={{ color: "#3f3f46", fontSize: "11px" }}>—</span>;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
      {unique.map((r) => (
        <button
          ...
          onClick={(e) => {
            e.stopPropagation();
            router.push(`/asset-scanning/${r.scan_id}/${r.id}`);
          }}
          style={{
            display: "flex",
            alignItems: "center",
        ...
        >
          ...
          <span style={{ color: "#6366f1", fontSize: "10px" }}>
            {r.scan_id.slice(0, 8)}…
          </span>
        </button>
      ))}
...
export default function DashboardPage() {
  ...
  return (
    <div
      style={{ padding: "36px 40px", width: "100%", boxSizing: "border-box" }}
    >
      {showModal && (
        <UploadModal
      ...
      >
        <div>
          ...
          >
            Overview of your vulnerability management activity
          </p>
        </div>
        <button
      ...
      >
        <div
          style={{ padding: "16px 20px", borderBottom: "1px solid #1f1f2e" }}
        >
                ...
                {scans.map((scan) => (
                  <tr
                    ...
                    style={{
                      ...
                      cursor: "pointer",
                      background:
                        hovered === scan.id ? "#111118" : "transparent",
                      transition: "background 0.15s",
                    }}
                  >
                    <td style={{ ...TD, color: "white", fontWeight: 600 }}>
                      {scan.scan_name || scan.filename || "—"}
                    </td>
                    <td style={{ ...TD, color: "#71717a", fontSize: "12px" }}>
                      {scan.filename || "—"}
                    </td>
                    ...
                    >
                      {formatSecs(scan.total_asset_secs)}
                    </td>
                    <td style={TD}>
                    ...
                    >
                      <button
                        ...
                        style={{
                          ...
                          color: deleting === scan.id ? "#3f3f46" : "#52525b",
                          padding: "4px",
                          lineHeight: 0,
                        }}
                      >
