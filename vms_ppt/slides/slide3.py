...
def create_slide3(prs: Presentation, slide3_data):
    ...
    # **FIXED POSITIONING - NO DYNAMIC CALCULATIONS**
    ...
    # FIXED table height instead of dynamic calculation
    table1_height = Inches(2.0)  # Fixed height
    ...
    if table1_rows > 0:
        table1.rows[0].height = Inches(0.35)  # Header row
    for i in range(1, min(table1_rows, len(table1.rows))):
    ...
    # Table 2 - Potential Threats - FIXED POSITIONING
    table2_top = Inches(3.4)  # Fixed position to avoid overlap
    ...
    # FIXED table height instead of dynamic calculation
    table2_height = Inches(2.4)  # Fixed height
    table2 = slide.shapes.add_table(table2_rows, table2_cols, table_left, Inches(3.8), max_table_width, table2_height).table
    ...
    if table2_rows > 0:
        table2.rows[0].height = Inches(0.35)  # Header row
    for i in range(1, min(table2_rows, len(table2.rows))):
    ...
    # Table 3 - Risk and Impact - FIXED POSITIONING
    table3_top = Inches(6.4)  # Fixed position
    ...
    # Set row heights for table 3
    table3.rows[0].height = Inches(0.35)  # Header row
    if len(table3.rows) > 1:
        table3.rows[1].height = Inches(0.4)  # Data row (slightly taller for text)
