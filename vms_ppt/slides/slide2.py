...
def create_slide2(prs: Presentation, slide2_data):
    ...
    baseline_table = slide.shapes.add_table(2, 2, table_left, Inches(1.05), left_table_width, Inches(0.8)).table
    baseline_table.columns[0].width = Inches(3.5)
    baseline_table.columns[1].width = Inches(1.3)

    # Set row heights for baseline table
    baseline_table.rows[0].height = Inches(0.35)  # Header row
    baseline_table.rows[1].height = Inches(0.35)  # Data row
    ...
    # Footnote
    footnote1_box = slide.shapes.add_textbox(table_left, Inches(1.9), left_table_width, Inches(0.2))
    ...
    failures_table = slide.shapes.add_table(failures_rows, 2, table_left, Inches(2.45), left_table_width, Inches(1.4)).table
    failures_table.columns[0].width = Inches(3.5)
    failures_table.columns[1].width = Inches(1.3)

    # Set row heights for failures table
    for i in range(failures_rows):
        failures_table.rows[i].height = Inches(0.35)
    ...
    vuln_table = slide.shapes.add_table(vuln_rows, vuln_cols, right_table_left, Inches(0.8), right_table_width, Inches(1.8)).table
    ...
    # Set row heights for vulnerability table
    vuln_table.rows[0].height = Inches(0.35)  # Header row
    for i in range(1, vuln_rows):
        vuln_table.rows[i].height = Inches(0.3)  # Data rows
