    ...
    def _build_crew(llm):
        ...
        task_understand = Task(
            description=(
                ...
                "Build a complete mental model of all signals. While doing so, apply these critical checks:\n"
                "1. RTI TAGS: The 'rti' field is auto-generated by Qualys and is frequently wrong. "
                "Cross-check every RTI tag against the cve_description, kb_diagnosis, and kb_consequence. "
                "If an RTI tag (e.g. 'Privilege Escalation') does not match what the CVE actually does, "
                "flag it as a mismatch and note what the CVE actually does instead.\n"
                "2. EXPLOIT COUNT INFLATION: The exploit list often contains patch commits, fix commits, "
                "security advisories, and vendor announcements that are NOT exploits. "
                "For each entry in unique_exploits, check its url, name, and notes — "
                "if it is a patch/fix/advisory/commit rather than attack code, mark it as non-exploit. "
                "Count only genuine attack tools or PoC code as real exploits.\n"
                "3. Everything else: note scores, timelines, asset context, role mismatches."
            ),
            expected_output=(
                "Structured observations covering: all data signals, any RTI tag mismatches with justification, "
                "and a cleaned exploit count separating real exploits from patches/advisories."
            ),
            agent=analyst,
        )
        task_correlate = Task(
            description=(
                "Using your observations, correlate across all three data blocks. "
                "Use your corrected RTI assessment and cleaned exploit count — not the raw values. "
                "Find contradictions, compounding risks, and non-obvious insights. "
                "If the RTI tag was wrong, explain what the actual vulnerability behaviour means for this asset. "
                "If the exploit count was inflated, explain what the true exploit landscape looks like and "
                "how that changes the real risk versus what the raw numbers suggest. "
                "Think freely — reason like an expert, not a checklist."
            ),
            expected_output=(
                "Correlated findings using corrected RTI and cleaned exploit data, "
                "including contradictions, compounding risks, and non-obvious insights."
            ),
            agent=analyst,
        ...
        task_output = Task(
            description=(
                ...
                "Use your corrected RTI and cleaned exploit count throughout — never the raw values. "
                "Return ONLY valid JSON with no extra text:\n"
                "{\n"
                '  "risk_score": <float 0-10, derived from your full analysis — not just CVSS>,\n'
                '  "risk_label": <"Critical"|"High"|"Medium"|"Low">,\n'
                '  "risk_summary": <string — your correlated insight using corrected data>,\n'
                '  "asset_domain": <string>,\n'
                '  "urgency": <"Immediate"|"High"|"Medium"|"Low">,\n'
                '  "rti_validated": <string — the corrected RTI description, or original if correct>,\n'
                '  "rti_mismatch": <true|false — whether the original RTI tag was wrong>,\n'
                '  "real_exploit_count": <int — count of genuine exploits after removing patches/advisories>,\n'
                '  "risk_factors": [<each item must connect signals from at least two different data blocks>],\n'
                '  "evidences": [<string>, ...]\n'
                "}"
            ),
            expected_output="Valid JSON risk assessment object with corrected RTI and exploit count.",
            agent=analyst,
